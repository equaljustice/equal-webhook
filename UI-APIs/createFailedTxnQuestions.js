...
export async function getFAILED_TXNDataAsJson() {
    ...
    try {
        ...
        const [questionsRows, fields1] = await connection.execute('SELECT * FROM FAILED_TXNquestions');
        for (const question of questionsRows) {

            // Fetch next question IDs
            const [nextIdsRows, fields2] = await connection.execute('SELECT nextQuestionId FROM FAILED_TXNnext_question_ids WHERE qid = ?', [question.qid]);


            let choicesArray = [];
            // Fetch answer choices for choice type questions
            if (question.answerType === 'choice') {
                const [choicesRows, fields3] = await connection.execute('SELECT choice FROM FAILED_TXNanswer_choices WHERE qid = ?', [question.qid]);
                for (const choice of choicesRows) {
                    choicesArray.push(choice.choice);
                }
            }
            else if(question.answerType == 'yesno'){
                choicesArray.push('Yes');
                choicesArray.push('No');
            ...
            if(nextIdsRows.length && nextIdsRows[0].nextQuestionId){
                console.log(nextIdsRows[0].nextQuestionId)
